  ...
  return (
    <main>
      ...
      <section className="library" id="library"><div className="shell">
        ...
        {visible.length ? <div className="resource-grid">{visible.map(item=><div className={`resource-card ${item.cover ? "has-cover" : ""} ${item.name === "PPT Generation" ? "ppt-reference-card" : ""}`} key={item.name} role="button" aria-label={`查看${item.name}案例`} onClick={()=>openResource(item)} tabIndex={0} onKeyDown={(event)=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();openResource(item)}}}>
