...
const resources: Resource[] = [
  ...
  { name:"PPT Generation", type:"Skill", description:"DeerFlow 的演示文稿生成 Skill：按统一视觉风格逐页生成幻灯片图像，并自动合成为专业 PPTX。", category:"内容与设计", tags:["PPTX","AI 生图","统一风格","DeerFlow"], icon:"PPT", tint:"#edf4ff", featured:true, cover:"/cases/deerflow-ppt-generation.png", caseTitle:"从内容大纲到统一视觉的完整演示文稿", caseSummary:"可规划多页结构，选择玻璃拟态、暗黑高级、现代渐变、Keynote 等视觉方向；每页会沿用前一页的视觉参考，使整套演示保持一致。", prompt:"使用 PPT Generation，基于这份大纲生成 10 页 16:9 的商务风格 PPTX，并保持整套视觉统一。", author:"ByteDance DeerFlow 团队", platform:"DeerFlow；可由 Codex、Claude Code、Cursor 等编码 Agent 按仓库说明安装和调用", githubUrl:"https://github.com/bytedance/deer-flow/tree/main/skills/public/ppt-generation" },
  ...
  { name:"OpenCLI", type:"Skill", description:"把已登录的网站和 Electron 桌面应用变成可确定执行的命令行能力，供 AI Agent 复用浏览器会话完成网页与桌面自动化。", category:"Codex 增强", tags:["浏览器自动化","Chrome 登录态","CLI","开源"], icon:"OC", tint:"#eaf2ff", featured:true, cover:"/cases/opencli-browser-bridge-gradient.png", caseTitle:"让 AI Agent 通过你已登录的浏览器操作真实服务", caseSummary:"OpenCLI 提供 100+ 网站适配器、浏览器基础操作和桌面应用 CDP 控制，可用于检索、填写表单、提取内容、截图及调用本地工具；不会在运行时额外消耗模型 Token。适合 Codex、Claude Code、Cursor 等支持 Skills 的 Agent 环境。", prompt:"使用 OpenCLI，打开并检查我当前已登录的网页，先告诉我将执行哪些步骤。", author:"jackwener（jakevin）", platform:"Codex、Claude Code、Cursor 等支持 Skills 的 AI Agent；Chrome / Electron 桌面应用", githubUrl:"https://github.com/jackwener/opencli" },
  ...
  return (
    <main>
      ...
      <section className="library" id="library"><div className="shell">
        ...
        {visible.length ? <div className="resource-grid">{visible.map(item=><div className={`resource-card ${item.cover ? "has-cover" : ""} ${item.name === "PPT Generation" ? "ppt-generation-card" : ""}`} key={item.name} role="button" aria-label={`查看${item.name}案例`} onClick={()=>openResource(item)} tabIndex={0} onKeyDown={(event)=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();openResource(item)}}}>
      ...
      {selected && <div className="modal-backdrop" role="presentation" onMouseDown={(e)=>{if(e.target===e.currentTarget)setSelected(null)}}><section className={`detail-modal ${selected.cover ? "has-cover" : ""} ${selected.name === "PPT Generation" ? "ppt-generation-detail" : ""}`} role="dialog" aria-modal="true" aria-label={`${selected.name}详情`}>
        <button className="modal-close" onClick={()=>setSelected(null)} aria-label="关闭">×</button>
        <div className="detail-media">
          {selected.cover ? <img className="detail-cover" src={selected.cover} alt={`${selected.name} 项目封面`}/> : currentMedia ? currentMedia.type === "image" ? <img src={currentMedia.src} alt={currentMedia.alt}/> : <video controls autoPlay muted playsInline><source src={currentMedia.src} type="video/mp4"/></video> : <div className="detail-placeholder" style={{background:selected.tint}}><span>{selected.icon}</span><p>{selected.category}</p></div>}
        ...
        <div className="detail-content"><div className="detail-meta"><span>{selected.type}</span><b>● 资源详情</b></div><h2>{selected.name}</h2><p className="detail-desc">{selected.description}</p><div className="detail-tags">{selected.tags.map(tag=><span key={tag}>#{tag}</span>)}</div>{selected.author && <div className="resource-facts"><p><span>作者</span>{selected.author}</p><p><span>适用平台</span>{selected.platform}</p><p><span>GitHub</span><a href={selected.githubUrl} target="_blank" rel="noreferrer">查看开源项目 ↗</a></p></div>}<div className="case-note"><small>CASE STUDY</small><h3>{selected.caseTitle}</h3><p>{selected.caseSummary}</p></div><div className="prompt-box"><small>在 Codex 中这样说</small><code>{selected.prompt}</code><button onClick={copyPrompt}>{copied?"已复制 ✓":"复制调用方式"}</button></div></div>
      </section></div>}
    </main>
  );
}
