  ...
  return (
    <main>
      ...
      <section className="library" id="library"><div className="shell">
        ...
        {visible.length ? <div className="resource-grid">{visible.map(item=><div className={`resource-card ${item.cover ? "has-cover" : ""}`} key={item.name} role="button" aria-label={`查看${item.name}案例`} onClick={()=>openResource(item)} tabIndex={0} onKeyDown={(event)=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();openResource(item)}}}>
      ...
      {selected && <div className="modal-backdrop" role="presentation" onMouseDown={(e)=>{if(e.target===e.currentTarget)setSelected(null)}}><section className={`detail-modal ${selected.cover ? "has-cover" : ""}`} role="dialog" aria-modal="true" aria-label={`${selected.name}详情`}>
