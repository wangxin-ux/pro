...
const resources: Resource[] = [
  ...
  { name:"OpenCLI", type:"Skill", description:"把已登录的网站和 Electron 桌面应用变成可确定执行的命令行能力，供 AI Agent 复用浏览器会话完成网页与桌面自动化。", category:"Codex 增强", tags:["浏览器自动化","Chrome 登录态","CLI","开源"], icon:"OC", tint:"#eaf2ff", featured:true, media:[{type:"image",src:"/cases/opencli-browser-bridge-feature.png",alt:"OpenCLI Browser Bridge 项目展示图"}], caseTitle:"让 AI Agent 通过你已登录的浏览器操作真实服务", caseSummary:"OpenCLI 提供 100+ 网站适配器、浏览器基础操作和桌面应用 CDP 控制，可用于检索、填写表单、提取内容、截图及调用本地工具；不会在运行时额外消耗模型 Token。适合 Codex、Claude Code、Cursor 等支持 Skills 的 Agent 环境。", prompt:"先安装 OpenCLI Skill：npx skills add jackwener/opencli --skill opencli-browser；然后在 Codex 中说：使用 OpenCLI，打开并操作我当前已登录的网页，先告诉我将执行哪些步骤。", author:"jackwener（jakevin）", platform:"Codex、Claude Code、Cursor 等支持 Skills 的 AI Agent；Chrome / Electron 桌面应用", githubUrl:"https://github.com/jackwener/opencli" },
...
export default function Home() {
  ...
  return (
    <main>
      ...
      {selected && <div className="modal-backdrop" role="presentation" onMouseDown={(e)=>{if(e.target===e.currentTarget)setSelected(null)}}><section className={`detail-modal ${selected.name === "OpenCLI" ? "opencli-detail" : ""}`} role="dialog" aria-modal="true" aria-label={`${selected.name}详情`}>
        ...
        <div className="detail-media">
          ...
          {selected.name === "OpenCLI" && <img className="opencli-badge" src="/cases/opencli-bridge-icon.png" alt="OpenCLI 浏览器桥接图标"/>}
